...
var currentOpponent = 2; //Points at current opponent
...
var charSelectScreen = function () {
    ...
    fill(0, 0, 0);
    textSize(20);
    text("Choose your character!",98,108);
    textSize(18);
    text("Afterwards, use the arrows", 97, 140);
    text("keys to pick your moves!", 107, 164);
...
var checkDefeat = function () {
    if (creatures[currentPlayer].hp < 1) {
        scene = 5;
        animationCap = 500;
    }
};
...
var Person = function(config) { //Constructor for characters
    ...
    this.abilitynames = config.abilitynames;
    creatures.push(this);
};
...
var player1 = new Person ({ //Creates Gunnar character
    x: 28,
    y: 195,
    attack: 6,
    defense: 4,
    hp: 30,
    hpMax: 30,
    abilitynames: ["Attack","Defend", "Restore", "Big Attack"],
    picture: function() {
    ...
    ability1: function() {//Attack
        ...
        if (creatures[currentOpponent].defense < this.attack) { //Checks if attack does damage
...
var player2 = new Person ({ //Creates the player character
    ...
    abilitynames: ["Great Heal","Stat swap", "Galvanize", "Pierce"],
    ability1: function() {//Great Heal
        if (healCounter < 3) {
            text("You healed yourself greatly, adding 20 hp!",100,370);
            this.hp += 20;
            fill(0, 255, 255);
            animationCap = 100;
            scene = 4;
    ...
    ability2: function() {//Stat swap
        ...
        text("Your attack increased by 4, but defense decreased by 2!",90,390);
        this.defense-= 2;
        this.attack+= 4;
        fill(0, 0, 255);
        animationCap = this.defense*20;
    ...
    ability3: function() {//Galvanize
        text("You galvanized, adding 4 defense!",100,370);
        this.defense += 4;
        fill(0, 255, 255);
        animationCap = 100;
        scene = 4;
    },
    ability4: function() {//Piercing Attack
        fill(0, 0, 0);
        text("You attack for " + this.attack + " damage!",100,370);
        text("It reduced their defense, but weakened you!",100,390);
        creatures[currentOpponent].defense -= 2;
        fill(200, 0, 0);
        if (creatures[currentOpponent].defense < this.attack) { //Checks if attack does damage
        creatures[currentOpponent].hp = creatures[currentOpponent].hp - (this.attack- creatures[currentOpponent].defense);
        }
...
var monster1 = new Person({ //Creates first enemy
    x: 224,
    y: 53,
    attack: 5,
    defense: 4,
    ...
    ability2: function() {
        text("Enemy stole your stats!",200,250);
        playSound(getSound("retro/hit2"));
        creatures[currentPlayer].defense--;
        creatures[currentPlayer].attack--;
        this.defense++;
        this.attack++;
        fill(0, 255, 0);
        animationCap = 100;
...
var monster2 = new Person({ //Creates second enemy
    x: 224,
    y: 61,
    attack: 8,
    defense: 4,
...
var finalBoss = new Person({ //Creates an enemy
    ...
    picture: function() {
        ...
        ability1: function() {
        fill(0, 0, 0);
        if (finalBossCharge === 1) {
            text("Enemy attacks for " + this.attack + " damage!",200,250);
            text("The enemy unleashed its attack!",200,270);
            if (this.attack > creatures[currentPlayer].defense) {
                creatures[currentPlayer].hp = creatures[currentPlayer].hp - (this.attack - creatures[currentPlayer].defense);
            ...
            text("The enemy charged its attack!", 200, 250);
    ...
    ability2: function() {
        text("The enemy halved your defense",200,250);
        creatures[currentPlayer].defense = ceil(creatures[currentPlayer].defense/2);
        fill(255, 0, 242);
        animationCap = creatures[currentPlayer].defense*20;
        scene = 4;
        playSound(getSound("retro/hit2"));
    },
});

var monsterAction = function () { //Determines what the monster does (if anything)
    if (turn === 1) {//If it's the monster's turn
        turn = 0; //Change turn
        if (creatures[currentOpponent].hp > 0) {//If hp isn't zero
            var randomAction = random(0, 2); //Determines random action
            if (randomAction > 1) {
        ...
        if (currentOpponent < 4) {
            currentOpponent++; //Sets current monster to the next one
            creatures[currentPlayer].hp = creatures[currentPlayer].hp + 4; //Heals player
...
var draw = function() {
    if(scene === 1) { //Select Screen
        charSelectScreen();
    }
    if (scene === 2) { //Draws the two characters
        gameBackground();
        if (finalBossCharge === 1) { //Final Boss Charge Indicator
            noFill();
            stroke(255, 255, 0);
            strokeWeight(20);
            ellipse(290, 140, 190, 190);
        }
        strokeWeight(1);
        stroke(0, 0, 0);
        ...
        rect(76,91,60,30);
        fill(0, 0, 0);
        text(creatures[currentPlayer].abilitynames[0], 78,35);
        text(creatures[currentPlayer].abilitynames[1], 147,72);
        text(creatures[currentPlayer].abilitynames[2], 87,111);
        text(creatures[currentPlayer].abilitynames[3], 19,70);
        if (currentPlayer === 1) { //Great Heal counter
            for (var j = 3 - healCounter; j > 0; j--) {
                fill(0, 255, 13);
                rect(80+j*12, 50, 5, 6);
            }
        }
    ...
    if (scene === 5) { //Defeat Screen
        frameRate(60);
        fill(0, 0, 0);
        animation = animation + 10;
        ellipse(200, 200, animation, animation);
        if (soundCounter === 0) {
        ...
        if (animation > animationCap) {
            ...
            image(getImage("creatures/OhNoes"), 10, 129);
            text("Enemies Defeated:" + (currentOpponent - 2), 115, 264);
        }

    }
    if (scene === 6) {//Victory Screen
        frameRate(60);
        fill(29, 250, 0);
        animation = animation + 10;
        ellipse(200, 200, animation, animation);
        ...
        if (animation > animationCap) {
            ...
            creatures[currentPlayer].draw();
            fill(13, 0, 255);
            text("Final Stats:", 22, 161);
        }
    }
    if (creatures[currentPlayer].defense < 0) {
        creatures[currentPlayer].defense = 0;
    }
    if (creatures[currentOpponent].defense < 0) {
        creatures[currentOpponent].defense = 0;
    }
};
